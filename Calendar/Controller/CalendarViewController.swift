...
class CalendarViewController: UIViewController {
    ...
    @IBOutlet weak var scrollCalendarBackwardButton: UIButton!
    @IBOutlet weak var scrollCalendarForwardButton: UIButton!
...
//    private var dateRange = DateRange(startDate: Date().addYear(-1),
    ...
    var selectedDate: Date? {
        willSet {
            if let isDateRepeated = self.selectedDate?
                ...
                if !isDateRepeated {
                    // only trigger date when previous selected date not same
//                    self.selectDate(date: newValue)
    ...
    override func viewDidLoad() {
        ...
        self.setupViewModel()
        self.setupTableView()
//        self.viewModel.setupData(selectedDate: Date().d)
...
//        print(self.dateRange.startDate)
        self.scrollToDate(date: Date(),animateScroll: false)
...
//        self.viewModel.goToTodaysDate(currentDate: date)
    }
...
//            self.scrollCalendarBackwardButton
//                .isEnabled = !date.isEqualMonthYear(self.dateRange.startDate)
//            self.scrollCalendarForwardButton
//                .isEnabled = !date.isEqualMonthYear(self.dateRange.endDate)
        }
